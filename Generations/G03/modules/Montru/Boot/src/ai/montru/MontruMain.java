...
public class MontruMain extends Giskard implements GiskardConsts {
	...
	public static void main(String[] args) {
		...
		try {
			...
			((GiskardNativeLogic)RUNTIME).gisExecLogic();
...
//	}
	...
	protected <ModAgent extends GiskardModule> ModAgent loadModule(String moduleName, String moduleVersion)
		...
		if ( null != ml ) {
			ret = ml.newInstance();
			Iterable<GiskardEntityRef> initTasks = ret.gisInitModule(RUNTIME, moduleName, moduleVersion);
			
			if ( null != initTasks ) {
				for ( GiskardEntityRef r : initTasks ) {
					addInitTask(r);
				}
			}
