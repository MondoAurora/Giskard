...
public class DustNodeUtils implements DustNodeConsts {
	...
	public static GiskardNativeLogic selectAgent(GiskardEntityRef target) {
		GiskardNativeLogic a = Giskard.access(GiskardAccess.Peek, null, target, GIS_ATT_DUST_INSTANCE);
	...
	public static GiskardResponse call(GiskardEntityRef target, GiskardAction action) {		
		return selectAgent(target).gisExecLogic();
	}

	public static GiskardResponse execute(GiskardEntityRef target) {	
		GiskardNativeLogic a = selectAgent(target);
		GiskardResponse resp;
		
		for ( resp = a.gisExecLogic(); GiskardResponse.Read == resp; ) {
			resp = a.gisExecLogic();
		}
